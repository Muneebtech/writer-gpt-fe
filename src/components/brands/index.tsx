...
const Brands = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (isSuccessDelete && DeletedId) {
      if (DeletedId) {
        const updatedChannelsList = filtered.filter(
          (channel: any) => channel.id !== DeletedId
        )
        filtered = [...updatedChannelsList]
      }
    }
    if (selectedCategory) {
  ...
  useEffect(() => {
    if ( ChannelCreateSuccess) {
      refetch({ page: 1, limit: 10 } as any)
